...
size_t FindTrails(int x, int y, std::vector<std::string>& list);
...
int main()
{
	...
	public:
		...
		void Render()
		{
			...
			m_text.setPosition({ static_cast<float>(initial.x * m_font_size), static_cast<float>(initial.y * m_font_size) });
			m_window->draw(m_text);
		...
		size_t FindTrails(int start_x, int start_y, int level_to_find, std::vector<std::string>& list)
		{
			...
			std::unordered_set<shared::Cell, shared::Cell::HashFunction> closed;
			std::queue<shared::Cell> open;
			open.push(start);
			...
			while (!open.empty())
			{
				...
				for (size_t i{ 0 }; i < open_size; ++i)
				{
					...
					{
						auto nearby_cell{ cell + shared::directions[dir] };
						...
						try
						{
							...
							if (nearby_cell_level == level_to_find && std::find(visited.begin(), visited.end(), nearby_cell) == visited.end())
							{
								if (level_to_find == 9)
								{
									total += 1;
									visited.insert(nearby_cell);
									targets.insert(nearby_cell);
								}
								else
								{
									open.push(nearby_cell);
								}
							}
						}
						catch (...) { }
		...
		std::unordered_set<shared::Cell, shared::Cell::HashFunction> targets;
		int x{ 0 };
		int y{ 0 };		
	};
